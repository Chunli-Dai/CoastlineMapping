...
if  status==0&&~isempty(cmdout) %if .ntf file is found, tif file is produced and stored in orthworkdir.
...
if status2==0&&~isempty(cmdout2) %if .tif file is found.
...
metafile=stripmetafile; %incase ntf not found.
...
if isempty(r)
	fprintf(['\n ',metafile,' data format not compatible.\n'])
Mstrip=struct(); Mstrip.x=[];Mstrip.y=[];Mstrip.z=[];Mstrip.coast=[];
return
end
...
Md1 = imdilate(wm.z, ones(widmstat/resrc)); %make sure water is absolutely water.
wm1 = interp2(wm.x,wm.y,Md1,data.x,data.y','*nearest',1);
ndwil=NDWI(wm1==0);
mean1=nanmean(ndwil);std1=nanstd(ndwil);cnt1=sum(~isnan(ndwil));
Me1=imerode(wm.z, ones(widmstat/resrc)); %land
