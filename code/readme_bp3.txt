...
For mono images:
Steps:
1\ add path (for SETSM, and imagery_utils) to your .bashrc file
export PATH=$PATH:~/landslide/code1/SETSM:/nobackupp17/elarour/coastlinedata/code/imagery_utils/
...
export LD_LIBRARY_PATH=~/landslide/code1/SETSM/tools/tiff-4.0.3/lib:$LD_LIBRARY_PATH
export LD_LIBRARY_PATH=~/landslide/code1/SETSM/tools/libgeotiff-1.4.2/lib:$LD_LIBRARY_PATH
export LD_LIBRARY_PATH=~/landslide/code1/SETSM/tools/proj-5.1.0/lib:$LD_LIBRARY_PATH

2\ orthorectify imagery; Code by Erik at PGC (more details see notes_erik.txt).
   2a\ Load Python environment:
source /nobackupp17/elarour/coastlinedata/code/miniconda3/bin/activate /nobackupp17/elarour/coastlinedata/code/miniconda3/envs/pgc
   2b\ Orthorectify images;
	2b.1\ Copy ~/codec2/orthorectf.sh to your workdir, and Revise <input_imagery_dir> <output_ortho_dir>.
	2b.2\ Run command:
	nohup ./orthorectf.sh > out_ortho.txt &
	Which is based on:
python pgc_ortho.py <input_imagery_dir> <output_ortho_dir> --dem <dem_tif> --epsg 3413 --outtype UInt16 --stretch ns --resample near --no-pyramids
	e.g., python /nobackupp17/elarour/coastlinedata/code/imagery_utils/pgc_ortho.py /u/cdai/elarour/coastlinedata/imagery_by_region/arcticdem_31_alaska_south/ ~/work/orthorectwork/  --dem /nobackupp17/elarour/coastlinedata/EGM2008_Arctic.tif --epsg 3413 --outtype UInt16 --stretch ns --resample near --no-pyramids
	Computation time: 15042 images took 50 hours with 24 cpus, 2GB mem, 8GB virtual mem. 

3\ copy files (tilelist CoastTileMonoMain.m  constant.m job.pbs) from template directory to your work directory.
