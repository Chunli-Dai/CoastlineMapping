function  [Mstrip]=multispec(stripmetafile,wm)
...
%control parameters
widm=2e3; %buffer widm of the a priori coastline, e.g., 2km.
...
tiffile=strrep(mfile{is},'.tif','.tif');
%[status , cmdout ]=system(['ls ',multidir,'/*/',ntffile]);
[status , cmdout ]=system(['find ',multidir,' -name ',ntffile]); %returns empty for cmdout when not found.
[status2 , cmdout2 ]=system(['find ',multidir,' -name ',tiffile]); 
if ~isempty(cmdout) %if .ntf file is found.
    ntffile=deblank(cmdout);
    [dir,name,ext] =fileparts(ntffile);
%   tiffile=deblank([dir,'/',mfile{is}]);
    tiffile=deblank([orthworkdir,'/',mfile{is}]);
...
Md1 = imdilate(wm.z, ones(widm/resrc)); %make sure water is absolutely water.
wm1 = interp2(wm.x,wm.y,Md1,data.x,data.y','*nearest',1);
ndwil=NDWI(wm1==0);
mean1=nanmean(ndwil);std1=nanstd(ndwil);
Me1=imerode(wm.z, ones(widm/resrc)); %land
